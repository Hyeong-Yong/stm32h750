...
adc_interleaved_tbl_t adc_tbl[ADC_MAX_CH] = { { &hadc1, &hadc2, &hdma_adc1,
false, false }, };
...
bool adcInitInterleaved() {
	...
	if (HAL_ADCEx_Calibration_Start(adc_tbl[_DEF_INTERLEAVED].hadc_master,
	ADC_CALIB_OFFSET_LINEARITY, ADC_SINGLE_ENDED) != HAL_OK) {
		ret = false;
		cliPrintf("ADC master calibration failed");
	}

	if (HAL_ADCEx_Calibration_Start(adc_tbl[_DEF_INTERLEAVED].hadc_slave,
	ADC_CALIB_OFFSET_LINEARITY, ADC_SINGLE_ENDED) != HAL_OK) {
		ret = false;
